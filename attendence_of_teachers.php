...
if(!empty($_POST)){
	...
	//	print_r($row);
		...
		$sql="SELECT * FROM attendence_teacher WHERE date='$date'";
		$result=$dbh->query($sql);

		if(!($row2=$result->fetch() )){
			//echo "OK";
			echo $row["name"]."さん".$date."で入力しました";
			$sql="INSERT attendence_teacher (name, date) VALUES(:name, :date)";
...
//表示
$sql="SELECT * FROM attendence_teacher";
$result=$dbh->query($sql);
...
<form action="attendence_of_teachers.php" method="post">
	<select name="year">
		<option value = "<?=$y?>">
			<?php echo $y; ?>
		</option>
